 ...
 * Parses the meta description on each public profile page.
 */
const fs = require('fs');
...
function loadProxies() {
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    if (!host || !port || !username || !password) {
      console.warn(`Skipping invalid DEDICATED_PROXY_${i}`);
      continue;
    }
    proxies.push({
      host,
...
function parseDescription(text) {
  ...
  if (/Performing security verification|Enable JavaScript|HTTP ERROR 429/i.test(decoded)) {
    return null;
  }
...
function loadArtists(onlyMissing) {
  const source = JSON.parse(fs.readFileSync(SOURCE, 'utf8'));
  let existing = new Map();
  if (fs.existsSync(OUTPUT)) {
    for (const artist of JSON.parse(fs.readFileSync(OUTPUT, 'utf8'))) {
      existing.set(artist.handle.toLowerCase(), artist);
    }
  }
  return source.map((artist) => {
    const prev = existing.get(artist.handle.toLowerCase()) || {};
    return {
      name: artist.name,
  ...
  }).filter((artist) => !onlyMissing || !artist.description);
}

function writeOutput(artists) {
...
async function main() {
  ...
  const allArtists = loadArtists(false);
  const toFetch = loadArtists(onlyMissing);
  if (!toFetch.length) {
  ...
  const withDesc = allArtists.filter((a) => a.description).length;
  console.log(`\nSaved ${allArtists.length} artists to ${OUTPUT} (${withDesc} with descriptions)`);
}
